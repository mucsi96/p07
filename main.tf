...
locals {
  ...
  oauth2_proxy_image_version = "v7.15.3" #https://github.com/oauth2-proxy/oauth2-proxy/releases
...
module "setup_twingate_connector" {
  source           = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_twingate_connector?ref=v-51"
...
module "setup_twingate_access" {
  source            = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_twingate_access?ref=v-51"
  environment_name  = var.environment_name
...
module "provision_hetzner_server" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/provision_hetzner_server?ref=v-51"
...
module "setup_cluster" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_cluster?ref=v-51"
...
module "create_redis_namespace" {
  source           = "git::https://github.com/mucsi96/k8s-modules.git//modules/create_app_namespace?ref=v-51"
  environment_name = var.environment_name
  k8s_namespace    = "redis"
...
module "create_redis" {
  source        = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_redis?ref=v-51"
  k8s_name      = "redis"
  k8s_namespace = module.create_redis_namespace.k8s_namespace
}

module "setup_ingress_controller" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_ingress_controller?ref=v-51"

  environment_name           = var.environment_name
  subscription_id            = var.azure_subscription_id
  dns_zone                   = data.azurerm_key_vault_secret.dns_zone.value
  traefik_chart_version      = "41.2.0" #https://github.com/traefik/traefik-helm-chart/releases
  traefik_version            = "v3.7.10" #https://github.com/traefik/traefik/releases
...
module "setup_metrics_server" {
  source                       = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_metrics_server?ref=v-51"
  metrics_server_chart_version = "3.13.1" #https://github.com/kubernetes-sigs/metrics-server/releases
  metrics_server_image_version = "v0.9.0" #https://github.com/kubernetes-sigs/metrics-server/releases
  wait_for                     = module.setup_ingress_controller.traefik_ready
}

module "setup_k8s_dashboard" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_k8s_dashboard?ref=v-51"
  ...
  valid_email                = data.azurerm_key_vault_secret.letsencrypt_email.value
  headlamp_chart_version     = "0.44.0"  #https://github.com/headlamp-k8s/headlamp/releases
  headlamp_image_version     = "v0.44.0" #https://github.com/headlamp-k8s/headlamp/releases
  oauth2_proxy_chart_version = local.oauth2_proxy_chart_version
  oauth2_proxy_image_version = local.oauth2_proxy_image_version
  session_redis = {
    connection_url = module.create_redis.connection_url
...
module "setup_prometheus_operator_crds" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_prometheus_operator_crds?ref=v-51"
...
module "create_database" {
  source        = "git::https://github.com/mucsi96/k8s-modules.git//modules/create_postgres_database?ref=v-51"
  k8s_name      = "postgres1"
...
module "register_grafana_dashboard" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/register_webapp?ref=v-51"
...
module "setup_prometheus_operator" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_prometheus_operator?ref=v-51"
  ...
  valid_email                         = data.azurerm_key_vault_secret.letsencrypt_email.value
  kube_prometheus_stack_chart_version = "88.3.0" #https://github.com/prometheus-community/helm-charts/releases?q=kube-prometheus-stack
  oauth2_proxy_chart_version          = local.oauth2_proxy_chart_version
  oauth2_proxy_image_version          = local.oauth2_proxy_image_version
  session_redis = {
    connection_url = module.create_redis.connection_url
...
module "setup_loki" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_loki?ref=v-51"

  loki_chart_version  = "7.3.0" #https://github.com/grafana/loki/blob/main/production/helm/loki/Chart.yaml
  alloy_chart_version = "1.11.1" #https://github.com/grafana/helm-charts/releases?q=alloy
...
module "setup_cloudbeaver" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_cloudbeaver?ref=v-51"
...
module "setup_backup_app" {
  source                     = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_backup_app?ref=v-51"
  environment_name           = var.environment_name
...
module "setup_learn_language_app" {
  source                     = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_learn_language_app?ref=v-51"
  environment_name           = var.environment_name
  azure_location             = var.azure_location
  owner                      = local.owner
  k8s_host                   = module.setup_cluster.k8s_host
...
module "setup_hello_app" {
  source                     = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_hello_app?ref=v-51"
  environment_name           = var.environment_name
  azure_location             = var.azure_location
  owner                      = local.owner
  k8s_host                   = module.setup_cluster.k8s_host
...
module "setup_training_log_app" {
  source                     = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_training_log_app?ref=v-51"
  environment_name           = var.environment_name
  azure_location             = var.azure_location
  owner                      = local.owner
  k8s_host                   = module.setup_cluster.k8s_host
...
module "setup_bank_email_worker" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_bank_email_worker?ref=v-51"
...
module "setup_expense_tracker_app" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_expense_tracker_app?ref=v-51"
...
module "setup_library_app" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_library_app?ref=v-51"
...
#   source                     = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_party_app?ref=v-51"
#   environment_name           = var.environment_name
#   azure_location             = var.azure_location
#   owner                      = local.owner
#   k8s_host                   = module.setup_cluster.k8s_host
