...
# Keep provider credentials known while setup_cluster reapplies its playbooks.
...
provider "kubernetes" {
  host                   = data.azurerm_key_vault_secret.k8s_host.value
  client_certificate     = data.azurerm_key_vault_secret.k8s_client_certificate.value
  client_key             = data.azurerm_key_vault_secret.k8s_client_key.value
  cluster_ca_certificate = data.azurerm_key_vault_secret.k8s_cluster_ca_certificate.value
}
...
module "setup_twingate_connector" {
  source           = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_twingate_connector?ref=v-85"
...
module "setup_twingate_access" {
  source            = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_twingate_access?ref=v-85"
  environment_name  = var.environment_name
...
module "provision_server" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/provision_server?ref=v-85"
...
module "setup_cluster" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_cluster?ref=v-85"
  ...
  server_maintenance_github_repository       = "p07"
  server_maintenance_github_repository_owner = "mucsi96"
...
module "create_redis_namespace" {
  source        = "git::https://github.com/mucsi96/k8s-modules.git//modules/create_app_namespace?ref=v-85"
  k8s_namespace = "redis"
}

module "create_redis" {
  source        = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_redis?ref=v-85"
  k8s_name      = "redis"
  k8s_namespace = module.create_redis_namespace.k8s_namespace
}

module "setup_ingress_controller" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_ingress_controller?ref=v-85"
...
module "setup_monitoring_crds" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_monitoring_crds?ref=v-85"

  monitoring_crds_chart_version = "28.0.1" #https://github.com/prometheus-community/helm-charts/releases?q=prometheus-operator-crds
  wait_for                      = module.setup_ingress_controller.ingress_controller_ready
}

module "create_database" {
  source        = "git::https://github.com/mucsi96/k8s-modules.git//modules/create_postgres_database?ref=v-85"
  k8s_name      = "postgres1"
...
module "register_grafana_dashboard" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/register_webapp?ref=v-85"
...
module "setup_victoria_metrics" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_victoria_metrics?ref=v-85"
...
module "setup_victoria_logs" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_victoria_logs?ref=v-85"
  ...
  # pod logs and Faro browser telemetry to its compatible ingestion endpoint.
...
module "setup_backup_app" {
  source                = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_backup_app?ref=v-85"
  environment_name      = var.environment_name
...
module "setup_learn_language_app" {
  source                = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_learn_language_app?ref=v-85"
  environment_name      = var.environment_name
  azure_location        = var.azure_location
...
module "setup_hello_app" {
  source                = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_hello_app?ref=v-85"
  environment_name      = var.environment_name
  azure_location        = var.azure_location
...
module "setup_training_log_app" {
  source                 = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_training_log_app?ref=v-85"
  environment_name       = var.environment_name
  azure_location         = var.azure_location
...
module "setup_bank_email_worker" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_bank_email_worker?ref=v-85"
...
module "setup_expense_tracker_app" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_expense_tracker_app?ref=v-85"
...
module "setup_library_app" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_library_app?ref=v-85"
...
module "setup_cooking_app" {
  source = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_cooking_app?ref=v-85"
...
#   source                     = "git::https://github.com/mucsi96/k8s-modules.git//modules/setup_party_app?ref=v-85"
#   environment_name           = var.environment_name
#   azure_location             = var.azure_location
#   owner                      = local.owner
#   k8s_oidc_issuer_url        = module.setup_cluster.oidc_issuer_url
