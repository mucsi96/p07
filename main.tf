...
module "setup_ingress_controller" {
  ...
  traefik_chart_version      = "41.0.2" #https://github.com/traefik/traefik-helm-chart/releases
  traefik_version            = "v3.7.7" #https://github.com/traefik/traefik/releases
...
module "setup_k8s_dashboard" {
  ...
  valid_email                = data.azurerm_key_vault_secret.letsencrypt_email.value
  headlamp_chart_version     = "0.43.0"  #https://github.com/headlamp-k8s/headlamp/releases
  headlamp_image_version     = "v0.43.0" #https://github.com/headlamp-k8s/headlamp/releases
  oauth2_proxy_chart_version = local.oauth2_proxy_chart_version
  oauth2_proxy_image_version = local.oauth2_proxy_image_version
  session_redis = {
    connection_url = module.create_redis.connection_url
...
module "setup_prometheus_operator_crds" {
  ...
  prometheus_operator_crds_chart_version = "30.0.1" #https://github.com/prometheus-community/helm-charts/releases?q=prometheus-operator-crds
  wait_for                               = module.setup_ingress_controller.traefik_ready
}
...
module "setup_prometheus_operator" {
  ...
  valid_email                         = data.azurerm_key_vault_secret.letsencrypt_email.value
  kube_prometheus_stack_chart_version = "87.16.1" #https://github.com/prometheus-community/helm-charts/releases?q=kube-prometheus-stack
  oauth2_proxy_chart_version          = local.oauth2_proxy_chart_version
  oauth2_proxy_image_version          = local.oauth2_proxy_image_version
  session_redis = {
    connection_url = module.create_redis.connection_url
...
module "setup_loki" {
  ...
  loki_chart_version  = "7.1.0"  #https://github.com/grafana/loki/blob/main/production/helm/loki/Chart.yaml
  alloy_chart_version = "1.10.1" #https://github.com/grafana/helm-charts/releases?q=alloy
...
#   source                     = "${local.module_source_base}/setup_party_app?ref=${local.module_source_ref}"
#   environment_name           = var.environment_name
#   azure_location             = var.azure_location
#   owner                      = local.owner
#   k8s_host                   = module.setup_cluster.k8s_host
